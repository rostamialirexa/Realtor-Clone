...
export default function ListingItem({ listing, id,onDelete,onEdit}) {
  console.log(listing)
  return (
    <li className="bg-white flex flex-col justify-between items-center shadow-md hover:shadow-lg rounded-md overflow-hidden transition-shadow duration-150 ease-in-out relative m-[10px]">
      <Link className="contents" to={`/category/${listing.type}/${id}`}>
        ...
        <Moment className="absolute top-2 left-2 bg-[#3377cc] text-white rounded-md uppercase text-xs font-semibold px-2 py-1 shadow-lg" fromNow>{listing.timestamp?.toDate()}</Moment>
